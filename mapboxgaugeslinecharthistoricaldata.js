...
function updateLineChart(cultivate_id,depth_id){['npkMinMax','moistMinMax','pHMinMax','tempMinMax'].forEach(id=>{const element=document.getElementById(id);if(element)element.remove();});if(!npkChart||!moistChart||!pHChart||!tempChart){return;}
axios.post('https://api-router.enfarm.com/api/v3/charts/retrieve-nutrition-chart',{cultivate_id:cultivate_id}).then(function(response){const responseContent=response.data.content;let data;const depthData=responseContent.find(item=>item.in_depth==depth_id);if(depthData){data=depthData.values;}else{console.log(`No data found for cultivateId: ${cultivate_id} and depthId: ${depth_id}`);return;}
