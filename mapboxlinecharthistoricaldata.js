...
function createOrUpdateGaugeCharts(latestNpkValue,latestMoistValue,latestPHValue,latestTempValue){const npkMaxValue=3;const npkThresholds=[0.5,1];const latestNpkRatio=latestNpkValue/300;if(npkGauge){npkGauge.destroy()}
...
moistGauge=setupGaugeChart("moistGauge","Moisture (%)",moistureThresholds,latestMoistValue,0,100,{colors:["#BA0F30","#18A558","#BA0F30"],thresholdLabels:["Insufficient","Adequate","Excess"],unit:'%'});const pHThresholds=[4,4.5,6.8,7,7.01,8];if(pHGauge){pHGauge.destroy()}
pHGauge=setupGaugeChart("pHGauge","pH Level",pHThresholds,latestPHValue,0,14,{colors:["#BA0F30","#BA0F30","#BA0F30","#18A558","#BA0F30","#BA0F30","#BA0F30"],thresholdLabels:["Very Acidic","Acidic","Slightly Acidic","Neutral","Slightly Alkaline","Alkaline","Very Alkaline"]});const tempThresholds=[20,30];if(tempGauge){tempGauge.destroy()}
tempGauge=setupGaugeChart("tempGauge","Temperature (°C)",tempThresholds,latestTempValue,0,50,{colors:["#BA0F30","#18A558","#BA0F30"],thresholdLabels:["Low Temp","Normal Temp","High Temp"],unit:'°C'})}
