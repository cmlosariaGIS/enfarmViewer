let currentFarmData=[];let userProfiles={};let uniqueFarmUsers=[];let userThumbnails={};const toggleFarmList=()=>{const farmListPopup=document.getElementById('popup-farmlist');farmListPopup.classList.toggle('show');if(farmListPopup.classList.contains('show')){fetchFarmListData(authenticatedUserIDs)}};const hideFarmList=()=>{document.getElementById('popup-farmlist').classList.remove('show')};const flyToFarmLocation=async(farmName)=>{try{const response=await fetch('https://api-ma.enfarm.com/api/v1/ma/get-install-locations',{headers:{'accept':'application/json'},});const{content:locations}=await response.json();const farmLocation=locations.find(location=>location.farmname===farmName);if(farmLocation){const latLng=L.latLng(farmLocation.long,farmLocation.lat);const zoomLevel=18;map.flyTo(latLng,zoomLevel,{animate:!0,duration:1});map.eachLayer(layer=>{if(layer instanceof L.Marker&&layer.getLatLng().equals(latLng)){layer.openPopup()}})}else{}}catch(error){}};const fetchUserProfile=async(userId)=>{try{const response=await fetch('https://api-router.enfarm.com/api/v2/user/get-user-profile',{method:'POST',headers:{'Content-Type':'application/json',},body:JSON.stringify({user_id:userId}),});const data=await response.json();if(data.status_code===200&&data.content){userProfiles[userId]=data.content}}catch(error){console.error('Error fetching user profile:',error)}};const fetchFarmListData=async(userIds)=>{try{const farmRequests=userIds.map(userId=>axios.post('https://api-router.enfarm.com/api/v3/farm/total-farms-per-user',{user_id:userId},{headers:{'accept':'application/json','Content-Type':'application/json'}}));const userRequests=userIds.map(userId=>fetch('https://api-router.enfarm.com/api/v2/user/get-user-profile',{method:'POST',headers:{'Content-Type':'application/json',},body:JSON.stringify({user_id:userId}),}).then(response=>response.json()));const thumbnailRequests=userIds.map(userId=>fetch('https://api-router.enfarm.com/api/v2/user/get-user-thumbnail',{method:'POST',headers:{'Content-Type':'application/json',},body:JSON.stringify({user_id:userId}),}).then(response=>response.json()));const[farmResponses,userResponses,thumbnailResponses]=await Promise.all([Promise.all(farmRequests),Promise.all(userRequests),Promise.all(thumbnailRequests)]);currentFarmData=farmResponses.flatMap((response,index)=>{const userId=userIds[index];const userProfile=userResponses[index].content;const userThumbnail=thumbnailResponses[index].content;userThumbnails[userId]=userThumbnail;return response.data.content.data.map(farm=>({...farm,userId:userId,userName:userProfile.user_name||'N/A',userPhone:userProfile.user_phone||'N/A'}))});uniqueFarmUsers=[...new Set(currentFarmData.map(farm=>farm.userName))];populateUserFilterOptions();displayFarmList(currentFarmData)}catch(error){console.error('Error fetching farm details:',error)}};const farmNeedsAttention=async(farm)=>{if(!farm.cultivates&&!farm.sensor_lst){console.warn('Farm has neither cultivates nor sensor_lst:',farm);return!1}
const itemsToCheck=farm.cultivates||farm.sensor_lst||[];const checkPromises=itemsToCheck.map(async item=>{const cultivateId=item.cultivate_id||item.id;if(!cultivateId){console.warn('No cultivate_id or id found for item:',item);return!1}
...
async function populateUserFilterOptions() {
    ...
    const thumbnailPromises = uniqueFarmUsers.map(async (user) => {
        ...
        if (userId) {
            try {
                const response = await fetch('https:method:'POST',headers:{'Content-Type':'application/json',},body:JSON.stringify({user_id:userId}),});const data=await response.json();if(data.status_code===200&&data.content){return{user,thumbnail:data.content}}}catch(error){console.error('Error fetching user thumbnail:',error)}}
return{user,thumbnail:null}});const thumbnailResults=await Promise.all(thumbnailPromises);thumbnailResults.forEach(({user,thumbnail},index)=>{const label=document.createElement('label');const input=document.createElement('input');input.type='radio';input.name='farm-user';input.value=user;input.id=`user-${index}`;const farmCount=userFarmCounts[user]||0;const farmText=farmCount===1?'farm':'farms';const container=document.createElement('div');container.style.display='flex';container.style.alignItems='center';if(thumbnail){const thumbnailImg=document.createElement('img');thumbnailImg.src=`data:image/jpeg;base64,${thumbnail}`;thumbnailImg.alt=`${user}'s thumbnail`;thumbnailImg.style.width='30px';thumbnailImg.style.height='30px';thumbnailImg.style.borderRadius='50%';thumbnailImg.style.marginRight='10px';container.appendChild(thumbnailImg)}
const userInfo=document.createElement('span');userInfo.textContent=`${user} (${farmCount} ${farmText})`;container.appendChild(userInfo);label.appendChild(input);label.appendChild(container);userFilterOptions.appendChild(label)})}
function copyToClipboard(text){if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(text).then(()=>{alert('Phone number copied to clipboard!')}).catch((err)=>{console.error('Failed to copy: ',err)})}else{let textArea=document.createElement("textarea");textArea.value=text;textArea.style.position="fixed";document.body.appendChild(textArea);textArea.focus();textArea.select();try{document.execCommand('copy');alert('Phone number copied to clipboard!')}catch(err){console.error('Failed to copy: ',err)}
document.body.removeChild(textArea)}}
async function fetchUserThumbnail(userId){try{const response=await fetch('https://api-router.enfarm.com/api/v2/user/get-user-thumbnail',{method:'POST',headers:{'Content-Type':'application/json',},body:JSON.stringify({user_id:userId}),});const data=await response.json();if(data.status_code===200&&data.content){userThumbnails[userId]=data.content}}catch(error){console.error('Error fetching user thumbnail:',error)}}
