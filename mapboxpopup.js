...
function updateHistoricalSoilDataPopup(event) {
    ...
    document.getElementById('address').textContent = farmAddress;
...
var images = [
    'https://i.ibb.co/C0gT2rp/projeto-cafe-gato-mourisco-Z8m-Gr-8-E7f4-unsplash.jpg',
    'https://i.ibb.co/HXbrp66/projeto-cafe-gato-mourisco-Uz1-Rk85-VUW4-unsplash.jpg',
    'https://i.ibb.co/sqXW5Lf/projeto-cafe-gato-mourisco-z-OVRgig-QMQA-unsplash.jpg',
    'https://i.ibb.co/D5rD1qj/projeto-cafe-gato-mourisco-eq5-OMTg-ED4-unsplash.jpg',
    'https://i.ibb.co/JH08gkp/projeto-cafe-gato-mourisco-6fo-Phyz6-QU-unsplash.jpg',
    'https://i.ibb.co/R0LYsq5/projeto-cafe-gato-mourisco-q-EJd-MO6-Qml-Y-unsplash.jpg',
    'https://i.ibb.co/4WC5LWz/projeto-cafe-gato-mourisco-Pjv-HCp-KPzho-unsplash.jpg',
    'https://i.ibb.co/gRdpBB3/anton-shuvalov-GTz-Rv-LR6a-OU-unsplash.jpg',
    'https://i.ibb.co/Lg2SCqc/projeto-cafe-gato-mourisco-yw-Ec-Uv-W4-Ha-U-unsplash.jpg',
    'https://i.ibb.co/ByhYLQy/projeto-cafe-gato-mourisco-b-Forvtl-Lx-SA-unsplash.jpg'
];
...
// Select a random image URL from the images array
const randomIndex = Math.floor(Math.random() * images.length);
const randomImage = images[randomIndex];

const getRandomImageUrl = () => images[Math.floor(Math.random() * images.length)];
...
const fetchFarmData3D = async (userIds) => {
            ...
            .then(async data => {
                ...
                const filteredLocations = data.content.filter(location => {
                    return !(/toàn|test|enfarm|koko/i.test(location.farmname) || /toàn|test|enfarm|koko/i.test(location.region_name))
                        && farmNames.includes(location.farmname);  // Only include farms for the specified users
                });
                ...
                // Count unique farms based on lat/long combinations
                const uniqueFarms = new Set();
                filteredLocations.forEach(location => {
                    const farmKey = `${location.lat},${location.long}`;
                    uniqueFarms.add(farmKey);
                });

                const uniqueFarmCount = uniqueFarms.size;

                // Update the totalfarmCount element with the count of unique farms
                ...
                `;
                    ...
                    // Add margin to move the entire element down
  ...
  <span class="tooltip-right" data-translate="See detailed soil data">See detailed soil data</span>
                                    ...
                                    const nutritionDataPromises = cultivateDetails.softids.map(async softid => {
                                        try {
                                            const nutritionData = await fetch(`https://api-router.enfarm.com/api/v3/charts/retrieve-nutrition-chart`, {
                                                ...
                                                .then(data => {
                                                    ...
                                                    if (matchingValues && matchingValues.created_at.length > 0) {
                                                        ...
                                                        return {
                                                            ...
                                                            nts: matchingValues.nts ? matchingValues.nts[latestIndex] : undefined,
                                                ...
                                                .catch(error => {
                                                    console.error(`Error fetching nutrition data for cultivate_id ${cultivate.cultivate_id} and in_depth ${softid.in_depth}:`, error);
                                                    return null;
                                                });
                                            ...
                                            if (nutritionData) {
                                                ...
                                                const roundedValue = (value) => value !== null && value !== undefined ? value.toFixed(2) : 'N/A';

                                                let needsAttention = circleColorTemp === '#BA0F30' ||
                                                    circleColorpH === '#BA0F30' ||
                                                    circleColorMoist === '#BA0F30' ||
                                                    circleColorNPK === '#BA0F30' ||
                                                    circleColorNTS === '#BA0F30' ||
    ...
    `;
                    ...
                    document.getElementById('needAttentionSum').innerText = farmsNeedingAttention.size;

                    // Update the totalDevicesCountFraction element with the total devices count
                    document.getElementById('totalDevicesCountFraction').innerText = uniqueFarmCount;
                    ...
                    /*cconst popup = new mapboxgl.Popup().setHTML(popupContent);
                    
                    onst marker = new mapboxgl.Marker(markerElement)
                        .setLngLat([location.long, location.lat])
                        .setPopup(popup)
                        .addTo(map);*/


                    //const popup = new mapboxgl.Popup().setText(translatePopupContent(popupContent)); // Use setText() instead of setHTML()
                    ...
                    .addTo(map);
                marker.originalContent = popupContent; // Store the original content
                allMarkers.push(marker);
                
                // Add click event listener to marker
                marker.getElement().addEventListener('click', () => {
                    console.log('Marker clicked, updating popup content');
                    const popup = marker.getPopup();
                    const updatedContent = translatePopupContent(marker.originalContent);
                    popup.setHTML(updatedContent);
                });*/
                    ...
                    marker.getElement().addEventListener('click', () => {
                        ...
                        // Add event listeners to chevron_forward icons after content is updated
                        setTimeout(() => {
                                ...
                                if (!btn.getAttribute('data-event-added')) {
                                    ...
                                    const handleClick = function () {
                                        try {
                                            ...
                                            // Find the closest farm type indicator
                                            const popupContent = this.closest('.mapboxgl-popup-content');
                                            if (!popupContent) {
                                            ...
                                            // Fetch the region_id
                                            const regionId = this.getAttribute('data-region-id') || 'Unknown';
                                            ...
                                            // Update the historical soil data popup
                                            updateHistoricalSoilDataPopup({ detail: eventDetail });
                                    
                                            // Dispatch custom event (if needed for other parts of your application)
                                            const event = new CustomEvent('soilDataRequested', { detail: eventDetail });
