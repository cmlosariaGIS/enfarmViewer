...
const originalIcon=createIcon('https://i.ibb.co/bWhk7NN/gps-shadow.png',[32,32],[0,-32]);const durianIcon=createIcon('https://i.ibb.co/nnrMSMd/durian-removebg-preview.png',[40,40],[-3,-28]);const coffeeIcon=createIcon('https://i.ibb.co/L6cGsSX/bean-shadow.png',[40,40],[-30,-40]);const pepperIcon=createIcon('https://i.ibb.co/WxVZrLN/icons8-pepper-96.png',[40,40],[-3,-30]);const teaIcon=createIcon('https://i.ibb.co/RCnz5gb/icons8-tea-leaves-64.png',[40,40],[-3,-30]);const warningIcon=createIcon('images/icons8-warning.gif',[30,30],[0,-40]);function applyFilters(){map.eachLayer(function(layer){if(layer instanceof L.MarkerClusterGroup){map.removeLayer(layer);}});let filteredMarkers=L.markerClusterGroup();const anyFilterActive=Object.values(activeFilters).some(filter=>filter);originalMarkers.eachLayer(function(marker){let shouldInclude=true;const popupContent=marker.getPopup().getContent();if(anyFilterActive){if(activeFilters.needsAttention&&!popupContent.includes('animation: glowRed')){shouldInclude=false;}
if((activeFilters.coffee&&marker.options.farmType!=='Coffee')||(activeFilters.durian&&marker.options.farmType!=='Durian')||(activeFilters.tea&&marker.options.farmType!=='Tea')||(activeFilters.pepper&&marker.options.farmType!=='Pepper')){shouldInclude=false;}}
if(shouldInclude){let iconUrl;if(!anyFilterActive){iconUrl='https://i.ibb.co/bWhk7NN/gps-shadow.png';}else if(activeFilters.needsAttention&&popupContent.includes('animation: glowRed')){iconUrl='images/icons8-warning.gif';}else{switch(marker.options.farmType){case'Coffee':iconUrl='https://i.ibb.co/L6cGsSX/bean-shadow.png';break;case'Durian':iconUrl='https://i.ibb.co/nnrMSMd/durian-removebg-preview.png';break;case'Tea':iconUrl='https://i.ibb.co/RCnz5gb/icons8-tea-leaves-64.png';break;case'Pepper':iconUrl='https://i.ibb.co/WxVZrLN/icons8-pepper-96.png';break;default:iconUrl='https://i.ibb.co/bWhk7NN/gps-shadow.png';}}
marker.setIcon(createCustomMarker(marker.options.farmName,iconUrl));const updatedPopupContent=updatePopupContent(popupContent);marker.getPopup().setContent(updatedPopupContent);filteredMarkers.addLayer(marker);}});map.addLayer(filteredMarkers);currentFilteredMarkers=filteredMarkers;}
