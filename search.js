const searchOptions=["..."," by farm name...",];const typingDelay=50;const eraseDelay=25;const pauseDelay=800;const searchInput=document.getElementById('searchInput');function displayTextWithTypingEffect(){let index=0;let isDeleting=!1;let text='';function type(){const currentText=searchOptions[index];if(isDeleting){text=currentText.substring(0,text.length-1)}else{text=currentText.substring(0,text.length+1)}
searchInput.placeholder="Search"+text;let typingSpeed=typingDelay;if(isDeleting){typingSpeed/=2}
if(!isDeleting&&text===currentText){isDeleting=!0;typingSpeed=pauseDelay}else if(isDeleting&&text===''){isDeleting=!1;index++;if(index===searchOptions.length){searchInput.placeholder="Search...";return}}
