...
function getNutrientRecommendation(status) {
    const statusKey = status.replace(/<[^>]*>/g, '').trim();
    //console.log("NPK status key:", statusKey);
    ...
    return recommendations[statusKey] || {
        en: "No specific recommendations available.",
        vi: "Không có khuyến nghị cụ thể."
    };
}
...
function getMoistureRecommendation(status) {
    const statusKey = status.replace(/<[^>]*>/g, '').trim();
    //console.log("Moisture status key:", statusKey);
...
function getPHRecommendation(status) {
    const statusKey = status.replace(/<[^>]*>/g, '').trim();
    //console.log("pH status key:", statusKey);
...
function getTempRecommendation(status) {
    const statusKey = status.replace(/<[^>]*>/g, '').trim();
    //console.log("Temperature status key:", statusKey);
...
function getNdtRecommendation(status) {
    const statusKey = status.replace(/<[^>]*>/g, '').trim();
    //console.log("NDT status key:", statusKey);

    const recommendations = {
        "Average NDT": {
            en: `
            • Continue current nitrogen application rates<br>
            • Monitor NDT levels bi-weekly<br>
            ...
            • Theo dõi mức NDT hai tuần một lần<br>
        ...
        "No data": {
            en: `
            • Check NDT sensors for malfunction or calibration issues<br>
            ...
            `,
            vi: `
            • Kiểm tra các cảm biến NDT xem có bị hỏng hoặc vấn đề hiệu chuẩn không<br>
    ...
    return recommendations[statusKey] || {
        en: "No specific recommendations available. Please consult with a local agronomist.",
        vi: "Không có khuyến nghị cụ thể. Vui lòng tham khảo ý kiến của chuyên gia nông học địa phương."
    };
}



function getP205Recommendation(status) {
    const statusKey = status.replace(/<[^>]*>/g, '').trim();
    //console.log("NDT status key:", statusKey);
    ...
    const recommendations = {
        "Average P2O5": {
            ...
            • Monitor NDT levels regularly<br>
            ...
            • Theo dõi mức NDT thường xuyên<br>
        ...
        "No data": {
            en: `
            • No recommendations at the moment. Check NDT monitoring equipment.
            `,
            vi: `
            • Hiện tại không có khuyến nghị. Kiểm tra thiết bị theo dõi NDT.
            `
        },
        "Không có dữ liệu": {
            en: `
...
function getk2oRecommendation(status) {
    const statusKey = status.replace(/<[^>]*>/g, '').trim();
    //console.log("K2O status key:", statusKey);
