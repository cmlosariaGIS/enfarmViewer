...
const userThumbnailApiUrl = 'https://api-router.enfarm.com/api/v2/user/get-user-thumbnail';
const farmThumbnailApiUrl = 'https://api-router.enfarm.com/api/v3/farm/retrieve-farm-detail';
...
async function getUserProfile(userId) {
    try {
        const response = await axios.post(userProfileApiUrl, { user_id: parseInt(userId) });
...
async function getUserThumbnail(userId) {
    try {
        const response = await axios.post(userThumbnailApiUrl, { user_id: parseInt(userId) });
        return response.data.content;
    } catch (error) {
...
function getTreeTypeName(treeType) {
    ...
    let key;
    switch (treeTypeInt) {
        case 0: key = "Coffee"; break;
        case 1: key = "Durian"; break;
        case 2: key = "Pepper"; break;
        case 3: key = "Tea"; break;
...
async function getFarmDetail(farmId) {
    try {
        const response = await axios.post(farmThumbnailApiUrl, { farm_id: parseInt(farmId) });
...
async function getRegionData(farmId) {
    try {
        const response = await axios.post(regionApiUrl, { farm_id: parseInt(farmId) });
        return response.data.content;
    } catch (error) {
...
async function getFarmData(userId) {
    try {
        ...
        document.getElementById('farmInfo').innerHTML = getTranslatedText('Fetching farm data...');

        const [userProfile, userThumbnail, farmResponse, farmDetailResponse, farmLocations] = await Promise.all([
        ...
        if (farmData.status_code === 200) {
            ...
            for (const farm of farms) {
                ...
                const farmThumbnail = farmDetailInfo && farmDetailInfo.farm_thumbnail
        ...
        <div class="combined-info">
            ...
            ${regionMap.size > 0 ? `
                <table>
                    ...
                    <tbody>
                        ${Array.from(regionMap.values()).map(region => `
                            <tr>
        ...
        <div class="box-info">
            ...
            ${boxData.is_have_box ? `
                <div class="box-cards">
                    ...
                    const boxImageUrl = box.box_type === 1 ? 'https://dashboard.enfarm.com/static/media/box_lora.5ae54139dc9704817c04.png' : 'https://dashboard.enfarm.com/static/media/ble.2dad651cc9747fa2ef39.png';
                    return `
                            <div class="box-card">
...
`;
            ...
            document.getElementById('farmInfo').innerHTML = `<p>${getTranslatedText('Error fetching farm data')}</p>`;
        }
    } catch (error) {
        console.error('Error:', error);
        document.getElementById('farmInfo').innerHTML = `<p>${getTranslatedText('Error fetching data')}</p>`;
    }
}
...
async function getSensorData(farmId) {
    try {
        const sensorResponse = await axios.post(sensorApiUrl, { farm_id: parseInt(farmId) });
...
async function getBoxData(farmId) {
    try {
        const boxResponse = await axios.post(boxApiUrl, { farm_id: parseInt(farmId) });
...
async function getRegionData(farmId) {
    try {
        const regionResponse = await axios.post(regionApiUrl, { farm_id: parseInt(farmId) });
