...
function generateTotalCounters(totalFarms,totalSensors,totalBoxes,totalGateways,){return`
  ...
  `;}
function generateCounters(sensorCount,boxCount,gatewayCount){return`
  ...
  `;}
function getSensorThumbnail(sensorType){if(sensorType.startsWith('3 (N')){return"https://dashboard.enfarm.com/static/media/sensor_3.bd41c965af3eb6bcdab4.png";}else if(sensorType.startsWith('7 (N')){return"https://dashboard.enfarm.com/static/media/sensor_7.e027c4034838eabc6495.png";}else{const lowerType=sensorType.toLowerCase();if(lowerType.includes("3")||lowerType.includes("npk")){return"https://dashboard.enfarm.com/static/media/sensor_3.bd41c965af3eb6bcdab4.png";}else if(lowerType.includes("7")||lowerType.includes("npkphcth-s")){return"https://dashboard.enfarm.com/static/media/sensor_7.e027c4034838eabc6495.png";}else{return"";}}}
...
async function getGatewayData(farmId){try{const gatewayResponse=await axios.post(gatewayApiUrl,{farm_id:parseInt(farmId),});return gatewayResponse.data.content;}catch(error){console.error("Error fetching gateway data:",error);return{is_have_gateway:false,gateways:[]};}}
function getTimeSinceActivation(activateDay){if(!activateDay){return'N/A';}
const now=new Date();const activationDate=new Date(activateDay.split('/').reverse().join('-'));const diffTime=Math.abs(now-activationDate);const diffDays=Math.ceil(diffTime/(1000*60*60*24));if(diffDays<7){return`${diffDays} <span data-translate="${diffDays !== 1 ? 'days' : 'day'}">${diffDays !== 1 ? 'days' : 'day'}</span> <span data-translate="ago">ago</span>`;}else if(diffDays<30){const weeks=Math.floor(diffDays/7);return`${weeks} <span data-translate="${weeks !== 1 ? 'weeks' : 'week'}">${weeks !== 1 ? 'weeks' : 'week'}</span> <span data-translate="ago">ago</span>`;}else if(diffDays<365){const months=Math.floor(diffDays/30);return`${months} <span data-translate="${months !== 1 ? 'months' : 'month'}">${months !== 1 ? 'months' : 'month'}</span> <span data-translate="ago">ago</span>`;}else{const years=Math.floor(diffDays/365);return`${years} <span data-translate="${years !== 1 ? 'years' : 'year'}">${years !== 1 ? 'years' : 'year'}</span> <span data-translate="ago">ago</span>`;}}
...
async function getFarmData(userId){try{document.getElementById("userId").textContent=userId;document.getElementById("farmInfo").innerHTML=getTranslatedText("Fetching farm data...",);const[userProfile,userThumbnail,farmResponse,farmDetailResponse,farmLocations,]=await Promise.all([getUserProfile(userId),getUserThumbnail(userId),axios.post(farmApiUrl,{user_id:parseInt(userId)}),axios.post(farmDetailApiUrl,{user_id:userId}),fetchFarmLocations(),]);const farmData=farmResponse.data;const farmDetailData=farmDetailResponse.data.content;if(farmData.status_code===200){let html="";if(userProfile){const thumbnailSrc=userThumbnail?`data:image/jpeg;base64,${userThumbnail}`:"images/temp_mascot.jpg";html+=`
                  ...
                  `;}
let totalFarms=0;let totalSensors=0;let totalBoxes=0;let totalGateways=0;const farms=farmData.content.data;totalFarms=farms.length;for(const farm of farms){const sensorData=await getSensorData(farm.farm_id);const boxData=await getBoxData(farm.farm_id);const gatewayData=await getGatewayData(farm.farm_id);totalSensors+=sensorData.is_have_sensor?sensorData.sensors.length:0;totalBoxes+=boxData.is_have_box?boxData.boxes.length:0;totalGateways+=gatewayData.is_have_gateway?gatewayData.gateways.length:0;}
console.log(`Total Farms: ${totalFarms}, Total Sensors: ${totalSensors}, Total Boxes: ${totalBoxes}, Total Gateways: ${totalGateways}`,);html+=generateTotalCounters(totalFarms,totalSensors,totalBoxes,totalGateways,);for(const farm of farmData.content.data){const farmDetail=farmDetailData[farm.farm_id]||{};const sensorData=await getSensorData(farm.farm_id);const boxData=await getBoxData(farm.farm_id);const gatewayData=await getGatewayData(farm.farm_id);const farmDetailInfo=await getFarmDetail(farm.farm_id);const regionData=await getRegionData(farm.farm_id);const cultivateData=farm.cultivates||[];const farmThumbnail=farmDetailInfo&&farmDetailInfo.farm_thumbnail?farmDetailInfo.farm_thumbnail:"images/noimage.jfif";const regionMap=new Map();regionData.forEach((region)=>{regionMap.set(region.region_name,{...region,cultivates:[],});});cultivateData.forEach((cultivate)=>{const region=regionMap.get(cultivate.name);if(region){region.cultivates.push(cultivate);}else{regionMap.set(cultivate.name,{region_id:"-",region_name:cultivate.name,method:"-",last_active:"-",cultivates:[cultivate],});}});html+=`
  <div class="farm-card">
      ...
      <div class="farm-body">
          ...
          <div class="combined-info">
              ...
              ${regionMap.size > 0
                        ? `<table><thead><tr><th data-translate="Region ID">Region ID</th><th data-translate="Cultivate ID">Cultivate ID</th><th data-translate="Region Name">Region Name</th><th data-translate="Tree Type">Tree Type</th><th data-translate="Last Update">Last Update</th></tr></thead><tbody>${Array.from(regionMap.values()).map((region)=>`
                              <tr>
          ...
          <div class="sensor-info">
              ...
              ${sensorData.is_have_sensor
                        ? `<div class="sensor-cards">${await Promise.all(sensorData.sensors.map(async(sensor)=>{const sensorDetails=await getSensorDetails(userId,sensor.sensor_qr_str);const activationTime=sensorDetails?getTimeSinceActivation(sensorDetails.activate_day):'';return`
    <div class="sensor-card">
        ...
        <div class="device-details">
            ...
            ${sensorDetails ? `<p><span data-translate="Region Deployed">Region Deployed</span>:${sensorDetails.region_name}</p><p><span data-translate="Depth">Depth</span>:${sensorDetails.in_depth_label}(${sensorDetails.in_depth})</p><p><span data-translate="Activated">Activated</span>:${sensorDetails.activate_day}<br>(${activationTime})</p>` : ''}
        </div>
    </div>
`;})).then(cards=>cards.join(''))}</div>`
        ...
        ${boxData.is_have_box
                        ? `<div class="box-cards">${await Promise.all(boxData.boxes.map(async(box)=>{const boxDetails=await getBoxDetails(userId,box.box_qr_string);const boxTypeText=box.box_type===1?"Phát đáp Lora":"Phát đáp BLE";const boxImageUrl=box.box_type===1?"https://dashboard.enfarm.com/static/media/box_lora.5ae54139dc9704817c04.png":"https://dashboard.enfarm.com/static/media/ble.2dad651cc9747fa2ef39.png";const activationTime=boxDetails?getTimeSinceActivation(boxDetails.activate_day):'';return`
      <div class="box-card">
        ...
        <div class="device-details">
          ...
          ${boxDetails ? `<p><span data-translate="Activated">Activated</span>:${boxDetails.activate_day}<br>(${activationTime})</p>` : ''}
        </div>
      </div>
    `;})).then(cards=>cards.join(''))}</div>`
  ...
  ${gatewayData.is_have_gateway
                        ? `<div class="gateway-cards">${await Promise.all(gatewayData.gateways.map(async(gateway)=>{const gatewayDetails=await getGatewayDetails(userId,gateway.gateway_qr_string);const activationTime=gatewayDetails?getTimeSinceActivation(gatewayDetails.activate_day):'';return`
          <div class="gateway-card">
            ...
            <div class="device-details">
              ...
              ${gatewayDetails ? `<p><span data-translate="Activated">Activated</span>:${gatewayDetails.activate_day}<br>(${activationTime})</p>` : ''}
            </div>
          </div>
        `;})).then(cards=>cards.join(''))}</div>`
...
async function getSensorDetails(userId,qrString){try{const detailResponse=await axios.post(sensorDetailApiUrl,{user_id:userId,qr_string:qrString,device_type:0,role:0});return detailResponse.data.content;}catch(error){console.error("Error fetching sensor details:",error);return null;}}
async function getBoxDetails(userId,qrString){try{const detailResponse=await axios.post(sensorDetailApiUrl,{user_id:userId,qr_string:qrString,device_type:1,role:0});return detailResponse.data.content;}catch(error){console.error("Error fetching box details:",error);return null;}}
async function getGatewayDetails(userId,qrString){try{const detailResponse=await axios.post(sensorDetailApiUrl,{user_id:userId,qr_string:qrString,device_type:2,role:0});return detailResponse.data.content;}catch(error){console.error("Error fetching gateway details:",error);return null;}}
...
function handleMicrochipClick(event){const userId=event.target.getAttribute('data-user-id');const farmId=event.target.getAttribute('data-farm-id');console.log(`User ID: ${userId}, Farm ID: ${farmId}`);try{console.log("Attempting to open user devices panel and expand farm card...");const userDevicesList=document.querySelector('.usersdevices-list');if(userDevicesList){userDevicesList.classList.add('show');console.log("User devices panel opened");}else{console.error("User devices list element not found");return;}
const handleFarmCard=(retryCount=0,maxRetries=5)=>{const farmCards=document.querySelectorAll('.farm-card');let farmCardFound=false;farmCards.forEach(card=>{const farmIdElements=card.querySelectorAll('.userfarm-details p strong');farmIdElements.forEach(element=>{if(element.textContent.includes('Farm ID:')||element.textContent.includes('ID Trang trại:')){const cardFarmId=element.nextSibling.textContent.trim();if(cardFarmId===farmId){const chevron=card.querySelector('.fas.fa-chevron-down, .fa-chevron-down');if(chevron){const isExpanded=card.classList.contains('active')||card.querySelector('.farm-body').style.display==='block';if(!isExpanded){chevron.click();console.log(`Farm card ${farmId} expanded`);}else{console.log(`Farm card ${farmId} is already expanded`);}
card.scrollIntoView({behavior:'smooth',block:'start'});farmCardFound=true;}else{console.error(`Chevron not found for farm ${farmId}`);}}}});});if(!farmCardFound&&retryCount<maxRetries){console.log(`Farm card not found. Retrying... (${retryCount + 1}/${maxRetries})`);setTimeout(()=>handleFarmCard(retryCount+1,maxRetries),1000);}else if(!farmCardFound){console.error(`Farm card not found for farm ${farmId} after ${maxRetries} attempts`);}};if(userId!=='admin'){setTimeout(()=>{const userSelect=document.getElementById('userSelect');if(userSelect){userSelect.value=userId;userSelect.dispatchEvent(new Event('change'));console.log(`User ${userId} selected in dropdown`);setTimeout(()=>handleFarmCard(),2000);}else{console.error("User select dropdown not found");}},500);}else{setTimeout(()=>handleFarmCard(),2000);}}catch(error){console.error("Error opening user devices panel or handling farm card:",error);}}
function toggleUserDevices(){const panel=document.querySelector(".usersdevices-list");if(panel){panel.classList.toggle("show");}}
function attachMicrochipClickListener(container){const microchipIcon=container.querySelector('.fi-bs-microchip');if(microchipIcon){microchipIcon.addEventListener('click',handleMicrochipClick);console.log("Microchip click listener attached");}else{console.error("Microchip icon not found in the container");}}
function attachPopupListeners(marker){marker.on("popupopen",function(){const popup=this.getPopup();const container=popup.getElement();if(container){attachMicrochipClickListener(container);}else{console.error("Popup container not found");}});}
markers.eachLayer(function(marker){attachPopupListeners(marker);});