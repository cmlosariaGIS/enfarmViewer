/*function updateUserProfile(userName = '', thumbnailBase64 = '') {
    ...
    if (userAvatar) {
        if (thumbnailBase64 && thumbnailBase64.startsWith('/9j/')) {
            console.log('Adding thumbnail to user profile');
            userAvatar.src = `data:image/jpeg;base64,${thumbnailBase64}`;
        } else {
            console.log('No thumbnail available, using default image');
            userAvatar.src = 'images/temp_mascot.jpg';
        }
    } else {
    ...
    if (usernameElement) {
        usernameElement.textContent = userName || translations[currentLang]['User'] || 'User';
    } else {
...
async function loginSuccess(accessToken, userId, userPhone) {
    ...
    if (ADMIN_PHONES.includes(userPhone)) {
        ...
        userName = 'enfarm Admin';
        thumbnailBase64 = ''; // You might want to set a default admin thumbnail here
        userEmail = 'admin@enfarm.com'; // Default admin email
        userDob = '';
        userGender = '';
    } else {
        try {
            ...
            if (userProfile.status_code === 200 && userProfile.content) {
                userName = userProfile.content.user_name || '';
                userEmail = userProfile.content.user_mail || '';
                userDob = userProfile.content.user_dob || '';
                userGender = userProfile.content.user_gender || '';
            }

            if (userThumbnail.status_code === 200 && userThumbnail.content) {
                thumbnailBase64 = userThumbnail.content;
            }
    ...
    // If userName is null, empty, or 'null', set it to 'User'
    if (!userName || userName === 'null') {
        userName = translations[currentLang]['User'] || 'User';
    }

    // Store user information in localStorage
    localStorage.setItem('userName', userName);
    localStorage.setItem('userThumbnail', thumbnailBase64);
    localStorage.setItem('userEmail', userEmail);
    localStorage.setItem('userDob', userDob);
    localStorage.setItem('userGender', userGender);

    // Update user profile
    updateUserProfile(userName, thumbnailBase64);
...
function showModal(messageKey, userId, userName = '', thumbnailBase64 = '') {
    ...
    if (thumbnailBase64 && thumbnailBase64.startsWith('/9j/')) {
        ...
        fullMessage += `<img src="images/temp_mascot.jpg" alt="User Thumbnail" style="width: 80px; height: 80px; border-radius: 50%; margin: 10px auto; display: block;">`;
    }

    if (userName) {
        fullMessage += `<div class="user-name" style="font-size: 20px; margin-top: 10px;">${userName}</div>`;
    } else {
        fullMessage += `<div class="user-name" style="font-size: 20px; margin-top: 10px;">${translations[currentLang]['User'] || 'User'}</div>`;
    }

    modalMessage.innerHTML = fullMessage;


    // Load and play the Lottie unlock animation
